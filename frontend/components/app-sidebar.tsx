...
export function AppSidebar() {
  ...
  return (
    <Sidebar className="font-manrope font-semibold">
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel className="font-clashDisplay font-semibold text-2xl text-center mx-auto text-text dark:text-text-dark my-5 tracking-wide">DCGPU Lab</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {
                items.map((item) => (
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              <SidebarMenuItem>
                <SidebarMenuButton asChild className="hover:cursor-pointer" onClick={toggleTheme}>
                  <div className="flex items-center justify-between text-sm h-8 " >
                    <div className="flex items-center gap-2">
                      <Moon size={16} />
                      <span>Dark</span>
                    </div>
                    <div className={cn("w-4 h-4 rounded-full text-center flex items-center justify-center", theme === "dark" ? "bg-blue-500" : "border border-slate-300")}>
                      {
                        theme === 'dark' && (
                          // <Check size={10} color="white" className="text-center" strokeWidth={4} />
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-4 text-white" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><motion.path d="M20 6 9 17l-5-5\" initial={{ pathLength: 0, pathOffset: 1 }} animate={{ pathLength: 1, pathOffset: 0 }} transition={{ duration: 0.2, ease: "easeInOut" }} /></svg>
                        )
                      }
                    </div>
                  </div>
